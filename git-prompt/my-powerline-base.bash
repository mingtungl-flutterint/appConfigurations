...
set_prompt_symbol() {
    # Foreground
    local f_coloroff='\[\e[0m\]'    # Text Reset
    local f_default='\[\e[39m\]'     # Default text color
    local f_black='\[\e[30m\]'       # Black

    local f_red='\[\e[31m\]'         # Red
    local f_green='\[\e[32m\]'       # Green
    local f_yellow='\[\e[33m\]'      # Yellow
    local f_blue='\[\e[34m\]'        # Blue
    local f_purple='\[\[\e[35m\]'      # Purple
    local f_cyan='\[\e[36m\]'        # Cyan
    local f_gray='\[\e[90m\]'        # Gray

    local f_lgray='\[\e[37m\]'       # Light Gray
    local f_lred='\[\e[91m\]'        # Light Red
    local f_lgreen='\[\e[1;32m\]'      # Light Green
    local f_lyellow='\[\e[93m\]'     # Light Yellow
    local f_lblue='\[\e[1;34m\]'       # Light Blue
    local f_lpurple='\[\e[95m\]'     # Light Purple
    local f_lcyan='\[\e[96m\]'       # Light Cyan
    local f_white='\[\e[97m\]'       # White

    # Background
    local b_default='\[\e[49m\]'     # Default bg color
    local b_black='\[\e[40m\]'       # Black
    local b_red='\[\e[41m\]'         # Red
    local b_green='\[\e[42m\]'       # Green
    local b_yellow='\[\e[43m\]'      # Yellow
    local b_blue='\[\e[44m\]'        # Blue
    local b_purple='\[\e[45m\]'      # Purple
    local b_cyan='\[\e[46m\]'        # Cyan
    local b_lgray='\[\e[47m\]'       # Light Gray

    local b_gray='\[\e[100m\]'       # Dark gray
    local b_lred='\[\e[101m\]'       # Light Red
    local b_lgreen='\[\e[102m\]'     # Light Green
    local b_lyellow='\[\e[103m\]'    # Light Yellow
    local b_lblue='\[\e[104m\]'      # Light Blue
    local b_lpurple='\[\e[105m\]'    # Light Purple
    local b_lcyan='\[\e[106m\]'      # Light Cyan
    local b_white='\[\e[107m\]'      # White

    #local symbol=$1
    #local color=$2
    #local prompt_symbol="$(enrich "true" $symbol $color)"
    #echo -n ${prompt_symbol}
    #printf "$(tput setaf 46)\U3bb\e[0m "
    echo -en "$f_lpurple\U3bb $f_coloroff"
}

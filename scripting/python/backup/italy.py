...
class ItalyLogCommon(LogBase):
    ...
    def _get_current_log_fullpath_pattern(self):
        if 'env_name' not in self.params:
...
class AamsITLog(ItalyLogCommon):
    ...
    def get_list_of_logs_fullpath(self, date:str, env:str)->list:
        ...
        if self.is_active_logs():
            #logs = self._get_current_log_fullpath() + '\\' + self.params['log_basename'] + '.log.' + self.params['date']
            #print ('>>> Active logs: \n', logs)
        ...
        if self.is_historical_logs():
            #logs = self._get_past_log_fullpath() + '\\' + self.params['log_basename'] + '.log.' + self.params['date'] + '.gz'
...
def collectStats(params:dict):
    ...
    processor.run({ 'log_object' : AamsITLog(params),
                    ...
                    'pattern' : re.compile(r'DBM_Q_OLAP_REPLICATE_USER_FLAGS_PRIVS_CHANGED:.*, docVerified=\'n\', privRMUdp=\'n\''),
...
if __name__ == '__main__':
    ...
    ap.add_argument('--env_name',     type=str, dest='env_name_list', required=False, default='IOM', help='Environments to process logs - IOM|QA')
